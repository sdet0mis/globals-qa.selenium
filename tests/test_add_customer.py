...
class TestAddCustomer(BaseTest):
    ...
    def test_add_customer(self):
        ...
        self.add_customer_page.enter_first_name(customer["first_name"])
        self.add_customer_page.enter_last_name(customer["last_name"])
        self.add_customer_page.enter_post_code(customer["post_code"])
        ...
        ), f"Клиент {customer['first_name']} не создан"
        customer_id = alert.text.split(":")[1]
        alert.accept()
        self.add_customer_page.click_customers_button()
        self.customers_page.fill_search_customer_field(customer["first_name"])
        self.customers_page.make_screenshot("Скриншот страницы клиентов")
        assert customer["first_name"] in (
            self.customers_page.get_customers_first_names()
        ), f"Клиент {customer['first_name']} не найден"
        self.customers_page.clear_search_customer_field()
        self.customers_page.click_delete_button(
            customer_id, customer["first_name"]
        )
        assert customer["first_name"] not in (
            self.customers_page.get_customers_first_names()
        ), f"Клиент {customer['first_name']} не удален"

    @allure.title("Создание клиента без фамилии")
    @allure.severity(allure.severity_level.CRITICAL)
    @pytest.mark.smoke
    def test_add_customer_without_last_name(self):
        self.manager_page.open_page()
        self.manager_page.click_add_customer_button()
        customer = new_customer()
        self.add_customer_page.enter_first_name(customer["first_name"])
